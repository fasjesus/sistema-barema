...
class PDFService:
    ...
    def _desenhar_capa(self, processo):
        ...
        for item in processo.itens:
            val_h = item.horas_validas
            
            # Formatação inteligente para remover o .0 de números inteiros
            if val_h > 0:
                h_str = f"{int(val_h)}" if val_h.is_integer() else f"{val_h}"
            else:
                h_str = ""
        ...
        c.setFont("Helvetica-Bold", 10)
        # Puxando o total direto do objeto ProcessoBarema
        c.drawString(x_cum - 1*cm, y_pos - 0.6*cm, f"TOTAL HORAS: {processo.total_horas}")
        c.save()
        packet.seek(0)
        return packet
